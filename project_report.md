# Project Report: Intelligent Lead Qualification System

**Version:** 1.0
**Date:** December 2025
**Author:** Lead Qualifier Team
...
The application follows a modern client-server architecture, designed for responsiveness and scalability.

### 2.1 High-Level Design
*   **Client (Frontend)**: A React.js Single Page Application (SPA) that handles user interaction, data visualization, and browser-based logic.
*   **Server (Backend)**: A lightweight Python Flask API that handles advanced machine learning inference and file persistence.
*   **Data Layer**: A hybrid approach using `localStorage` for session speed and JSON/Pickle files for server-side persistence.
...
g:/Lead Qualifier/
├── src/
│   ├── components/     # UI Modules (Dashboard, LeadCard, Settings)
│   ├── lib/            # Business Logic (agent.js, api.js)
│   └── App.jsx         # Main Controller & State Management
├── backend/
│   ├── app.py          # Flask API & Persistence Layer
│   ├── leads.json      # Persistent Data Store
│   └── model.pkl       # Active ML Model
```

### 4.2 Key API Endpoints
*   `GET /health`: Checks system status and model availability.
*   `POST /predict`: Performs inference using the loaded ML model.
*   `POST /upload-model`: Hot-swaps the active `.pkl` model file.
*   `GET/POST/DELETE /leads`: Manages the persistent lead database.

### 4.3 Data Persistence Strategy
To ensure data integrity, the system implements a robust sync mechanism:
*   **On Load**: App fetches `leads.json` from the server.
*   **On Update**: Any change (add/delete) is immediately synced to the server.
*   **Fallback**: If the server is offline, the app gracefully degrades to `localStorage`.
...
---
*Generated by Lead Qualifier Team*
